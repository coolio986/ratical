#!/usr/bin/env python3
"""Nginx auth_request helper: start Ratical configurator / crowsnest on first use,
then idle-stop them after a quiet period.

Endpoints (local only):
  GET /wake/configure  -> ensure ratical-configurator is up (port 3000)
...
LISTEN_HOST = os.environ.get("RATICAL_ONDEMAND_HOST", "127.0.0.1")
LISTEN_PORT = int(os.environ.get("RATICAL_ONDEMAND_PORT", "3199"))
IDLE_SEC = int(os.environ.get("RATICAL_ONDEMAND_IDLE_SEC", "900"))  # 15 min
POLL_SEC = int(os.environ.get("RATICAL_ONDEMAND_POLL_SEC", "30"))
START_TIMEOUT = int(os.environ.get("RATICAL_ONDEMAND_START_TIMEOUT", "120"))

SERVICES = {
    "configure": {
        "unit": "ratical-configurator.service",
        "host": "127.0.0.1",
        "port": int(os.environ.get("RATICAL_CONFIGURATOR_PORT", "3000")),
    },
    "crowsnest": {
        "unit": "crowsnest.service",
        "host": "127.0.0.1",
        "port": int(os.environ.get("RATICAL_CROWSNEST_PORT", "8080")),
    },
}
