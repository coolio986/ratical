...
# (sourced by install.sh). beacon is installed by ratical-install.sh in step 30.
...
if command -v ratical >/dev/null 2>&1; then
  as_user "ratical extensions register klipper linear_movement_analysis '${LMA_DIR}/linear_movement_vibrations.py'" \
    || warn "LMA extension registration failed"
  ok "linear_movement_analysis registered"
else
  warn "ratical CLI missing — cannot register LMA (rerun step 20 first)"
fi
...
if systemctl list-unit-files crowsnest.service &>/dev/null; then
  ...
  ok "crowsnest installed but disabled (nginx /webcam auto-wakes via ratical-ondemand)"
fi
