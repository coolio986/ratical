...
# via nginx auth_request → ratical-ondemand (idle-stops after RATICAL_ONDEMAND_IDLE_SEC).

# --- On-demand wake daemon (always enabled; tiny loopback HTTP helper) --------
ONDEMAND_DIR="/usr/local/lib/ratical-ondemand"
ONDEMAND_PY="${ONDEMAND_DIR}/ratical-ondemand-wake.py"
ONDEMAND_UNIT="/etc/systemd/system/ratical-ondemand.service"
SCRIPT_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"

report "Installing ratical-ondemand wake helper"
sudo mkdir -p "${ONDEMAND_DIR}"
sudo install -m 0755 "${SCRIPT_ROOT}/files/ratical-ondemand-wake.py" "${ONDEMAND_PY}"
sudo install -m 0644 "${SCRIPT_ROOT}/files/ratical-ondemand.service" "${ONDEMAND_UNIT}"
sudo systemctl daemon-reload
sudo systemctl enable --now ratical-ondemand.service
ok "ratical-ondemand active on 127.0.0.1:3199"

# upstream in http context (conf.d/*.conf is included by Debian's nginx.conf)
UPSTREAM_CONF="/etc/nginx/conf.d/ratical-configurator-upstream.conf"
report "Writing nginx upstream ${UPSTREAM_CONF}"
sudo tee "${UPSTREAM_CONF}" >/dev/null <<EOF
# added by ratical-kalico-generic
upstream ratical_configurator { server 127.0.0.1:${RK_CONFIGURATOR_PORT}; }
upstream ratical_ondemand { server 127.0.0.1:3199; }
EOF
...
WAKE_SNIPPET = """
    # Ratical on-demand wake (ratical-kalico-generic) — internal auth_request targets
    location = /ratical_wake_configure {
        internal;
        proxy_pass http://ratical_ondemand/wake/configure;
        proxy_pass_request_body off;
        proxy_set_header Content-Length "";
        proxy_connect_timeout 2s;
        proxy_read_timeout 120s;
    }
    location = /ratical_wake_crowsnest {
        internal;
        proxy_pass http://ratical_ondemand/wake/crowsnest;
        proxy_pass_request_body off;
        proxy_set_header Content-Length "";
        proxy_connect_timeout 2s;
        proxy_read_timeout 120s;
...
CONFIGURE_BLOCK = """
    # Ratical Configurator (on-demand via ratical-ondemand)
    location /configure {
        auth_request /ratical_wake_configure;
        proxy_pass http://ratical_configurator$request_uri;
...
def ensure_wake(s: str) -> str:
    if "location = /ratical_wake_configure" in s:
        return s
...
def replace_configure(s: str) -> str:
    ...
    anchor = "location = /ratical_wake_crowsnest"
    idx = s.find(anchor)
    if idx >= 0:
        end = s.find("}", idx)
        end = s.find("\n", end) + 1
...
def patch_webcam(s: str) -> str:
    def repl(m: re.Match) -> str:
        block = m.group(0)
        if "auth_request /ratical_wake_crowsnest" in block:
            return block
        return re.sub(
            r"(location /webcam/ \{\n)",
            r"\1        auth_request /ratical_wake_crowsnest;\n",
            block,
            count=1,
        )
