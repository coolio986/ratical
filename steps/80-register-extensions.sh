...
# file exists. The bundled ratical-install.sh registers extensions via a loop whose
# realpath fails for not-yet-installed files (LMA) and can miss the newer set
# (fastconfig, named_offsets, beacon_*, ratical_dual_carriage_extras). This step is the
# order-independent safety net: register every bundled extension, then symlink.

K="${RK_CONFIG}/Ratical/klippy"
[[ -d "${K}" ]] || { warn "klippy dir ${K} missing — did step 30 run?"; return 0; }
command -v ratical >/dev/null 2>&1 || { warn "ratical CLI missing — cannot register (rerun step 20)"; return 0; }

report "Registering all bundled klippy extras"
for py in "${K}"/*.py; do
  [[ -e "${py}" ]] || continue
  base="$(basename "${py}" .py)"
  # vaoc_led is experimental / registered on demand — skip to match Ratical.
  [[ "${base}" == "vaoc_led" ]] && continue
  as_user "ratical extensions register klipper '${base}' '${py}'" >/dev/null 2>&1 || true
done
...
for py in "${K}"/kinematics/*.py; do
  [[ -e "${py}" ]] || continue
  base="$(basename "${py}" .py)"
  as_user "ratical extensions register klipper -k '${base}' '${py}'" >/dev/null 2>&1 || true
done

report "Symlinking all registered extensions into klipper"
as_user "ratical extensions symlink" || warn "ratical extensions symlink failed — run manually"
