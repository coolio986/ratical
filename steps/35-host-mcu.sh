...
# (sourced by install.sh) — Ratical's initial-setup + V-Core configs use a [mcu]/[mcu rpi]
# linux process at /tmp/klipper_host_mcu. Without it klipper can't connect. Mirrors the
# Ratical rpi_mcu module.

FW="${RK_CONFIG}/Ratical/boards/rpi/firmware.config"

if systemctl list-unit-files 2>/dev/null | grep -q '^klipper_mcu.service'; then
  ok "klipper_mcu.service already installed"
  return 0
fi
[[ -f "${FW}" ]] || die "rpi firmware.config not found at ${FW} — did step 20/30 run? (config/Ratical must be linked)"
