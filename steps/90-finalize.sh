...
sudo tee /usr/local/bin/ratical-start-configure >/dev/null <<'H'
#!/bin/sh
exec sudo systemctl start ratical-configurator.service
H
sudo tee /usr/local/bin/ratical-start-vaoc >/dev/null <<'H'
#!/bin/sh
sudo systemctl start ratical-configurator.service
exec sudo systemctl start crowsnest.service
H
sudo tee /usr/local/bin/ratical-stop-vaoc >/dev/null <<'H'
#!/bin/sh
sudo systemctl stop crowsnest.service
if [ "$1" = "--all" ]; then
  sudo systemctl stop ratical-configurator.service
fi
H
sudo chmod 755 /usr/local/bin/ratical-start-configure /usr/local/bin/ratical-start-vaoc /usr/local/bin/ratical-stop-vaoc

report "Restarting core printer services"
sudo systemctl daemon-reload || true
for svc in klipper moonraker nginx ratical-ondemand; do
  sudo systemctl restart "${svc}" 2>/dev/null && ok "restarted ${svc}" || warn "could not restart ${svc}"
done
# Not enabled on boot — nginx /configure and /webcam wake them via ratical-ondemand.
sudo systemctl disable --now crowsnest.service 2>/dev/null || true
sudo systemctl disable --now ratical-configurator.service 2>/dev/null || true
...
cat <<EOF
  ...
  Auto on-demand (no manual start needed):
    http://<printer-ip>/configure/     → starts ratical-configurator
  ...
  Next:
    1) Open http://<printer-ip>/configure/
       ...
       This GENERATES Ratical.cfg. First load may take ~30–90s (cold start).
