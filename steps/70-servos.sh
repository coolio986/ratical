...
FORK_COPY="${RK_CONFIG}/Ratical/klippy/servo_enable_delay.py"
...
if command -v ratical >/dev/null 2>&1; then
  ...
  as_user "ratical extensions register klipper servo_enable_delay '${DEST}'" \
    || warn "servo_enable_delay registration failed (may already be registered)"
  as_user "ratical extensions symlink klipper" || warn "symlink failed"
  ok "servo_enable_delay registered + symlinked"
else
  warn "ratical CLI missing — cannot register servo_enable_delay (rerun step 20)"
fi
