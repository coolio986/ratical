...
then
	...
	"${KLIPPER_ENV}"/bin/python "${KLIPPER_DIR}"/scripts/graph_accelerometer.py -c /tmp/*_y_belt_tension.csv -o "${outdir}/y_tension_comparison_${DATE}.png"
fi
if [ $DISABLE_X -eq 0 ]
then
	...
	"${KLIPPER_ENV}"/bin/python "${KLIPPER_DIR}"/scripts/graph_accelerometer.py -c /tmp/*_x_belt_tension.csv -o "${outdir}/x_tension_comparison_${DATE}.png"
fi
