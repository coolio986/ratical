...
export const getBoards = async () => {
	...
	const defs = await glob(`${process.env.RATICAL_CONFIGURATION_PATH}/boards/*/board-definition.json`);
	const boards = defs
		.map((f) =>
			f.trim() === ''
				? null
...
export const compileFirmware = async <T extends boolean>(
	...
	try {
		...
		const firmwareDest = path.join(environment.RATICAL_DATA_DIR, binaryName);
...
const mcuMiddleware = middleware(async ({ ctx, next, meta, rawInput }) => {
	...
	try {
		...
		throw new TRPCError({
			code: 'INTERNAL_SERVER_ERROR',
			message: `Invalid board definition(s) in ${process.env.RATICAL_CONFIGURATION_PATH}/boards.`,
			cause: e,
		});
	}
	let board = null;
...
export const mcuRouter = router({
				...
				try {
					...
					try {
						const flashScript = path.join(
							current.path.replace(`${process.env.RATICAL_CONFIGURATION_PATH}/boards/`, ''),
							current.flashScript,
						);
	...
	flashViaPath: mcuProcedure
		...
		.mutation(async ({ ctx, input }) => {
			...
			try {
				const flashScript = path.join(
					ctx.board.path.replace(`${process.env.RATICAL_CONFIGURATION_PATH}/boards/`, ''),
