 ...
 *
 * 		; processed by Ratical 2.0.2-598-g1b6c63fa on 2024-11-07 at 15:08:23 UTC
 *
 *    The header line allows us to detect already-processed data during a streaming read (eg, processing
 *    during upload). The data values are considered informative only, versus the ratical_meta block which
 *    is considered authoritative.
 *
 * 2. The ratical_meta block. This is added at the end of the file. It comprises a 'begin' line, one or
 ...
 */
const rxRaticalMeta = /(?:^; ratical_meta begin (\d+)\n(.*))?\n; ratical_meta end (\d+)$/ms;

const rxGeneratorHeader =
	/^; generated (by|with) (?<GENERATOR>[^\s]+) (?<VERSION>[^\s]+) (in Ratical dialect (?<RATICAL_DIALECT_VERSION>[^\s]+) )?on (?<DATE>[^\s]+) at (?<TIME>.*)$/im;

const rxProcessedByRaticalHeaderPreV3 =
	/^; processed by Ratical (?<VERSION>[^\s]+) on (?<DATE>[^\s]+) at (?<TIME>\d\d:\d\d:\d\d(?:Z| UTC))/im;

const rxProcessedByRaticalHeaderV3 =
	/^; processed by Ratical\.PostProcessor (?<VERSION>[^\s]+) on (?<DATE>[^\s]+) at (?<TIME>\d\d:\d\d:\d\d(?:Z| UTC))(?: v:(?<V>3) m:(?<M>[\da-fA-F]+)(?<IDEX> idex)?)?(?:$|\s)/im;
...
export class GCodeFile {
	...
	/** The placeholder version used to represent files transformed by the legacy ratical.py post processor. */
	public static readonly LEGACY_RATICAL_VERSION = new SemVer('0.1.0-legacy');
	...
	private static getRaticalMetaFooter(analysisResult: AnalysisResult): string {
		...
		return `\n; ratical_meta begin ${b64.length}\n; ${chunks.join('\n; ')}\n; ratical_meta end ${chunks.length}`;
	}
	...
	public static async inspect(path: string, options: InspectOptions): Promise<GCodeFile> {
		...
		if (gci.fileFormatVersion === undefined) {
			...
			if (/^; processed by Ratical($|\s)/im.test(tail)) {
				gci.postProcessorVersion = GCodeFile.LEGACY_RATICAL_VERSION;
		...
		if (gci.fileFormatVersion !== undefined) {
			...
			if (gci.fileFormatVersion < GCodeFile.FILE_FORMAT_VERSION) {
				...
				reasons.push(
					'The file format is from an old version of Ratical which is no longer supported. The original file must be re-uploaded or re-sliced.',
				);
			} else if (gci.fileFormatVersion > GCodeFile.FILE_FORMAT_VERSION) {
				printability = Printability.PROCESSOR_NOT_SUPPORTED;
				reasons.push(
					'The file format is from a newer version of Ratical. Update Ratical, or re-upload or re-slice the original file.',
				);
			}
		}
		...
		if (gci.isProcessed) {
			if (gci.processedForIdex !== !!options.printerHasIdex) {
				...
				if (semver.eq(currentVersion, gci.postProcessorVersion)) {
					...
					reasons.push(
						'The file was processed by a more recent version of Ratical than the installed version. Either update Ratical, or the file must be reprocessed.',
					);
					printability = Printability.MUST_REPROCESS;
				} else if (currentVersion.major > gci.postProcessorVersion.major) {
					reasons.push(
						'There have been significant incompatible changes to Ratical gcode handling since the file was last processed.',
					);
					printability = Printability.MUST_REPROCESS;
				} else {
					reasons.push(
		...
		// NB: gci.raticalMetaFileOffset is set but not used yet. The code below is transitional and will be replaced.

		if (gci.isProcessed) {
			var tail = await fsReaderGetLines(path, -100);

			rxRaticalMeta.lastIndex = 0;
			var match = rxRaticalMeta.exec(tail);
			...
			if (match) {
				if (match[3] && !match[1]) {
					onWarning?.(
						...
						'Failed to parse ratical_meta block: the begin marker was not found.',
					);
				} else {
					...
					if (base64str.length != expectedBase64CharCount) {
						onWarning?.(
							WarningCodes.INVALID_METADATA,
							`Failed to parse ratical_meta block: expected ${expectedBase64CharCount} base64 characters, but found ${base64str.length}.`,
						);
					} else {
				...
				onWarning?.(WarningCodes.INVALID_METADATA, 'The ratical_meta block was not found.');
			}
		}
	...
	public async transform(outputFile: string, options: TransformOptions): Promise<GCodeInfo> {
		...
		try {
			...
			const gci = await gcodeProcessor.finalizeProcessing({
				...
				getProcessedByRaticalHeader: (currentCodeVersion, timestamp) =>
					GCodeFile.getProcessedByRaticalHeader({
						currentCodeVersion,
						timestamp,
						raticalMetaFileOffset: s.size,
			...
			await fh!.write(GCodeFile.getRaticalMetaFooter(gci.analysisResult));
	...
	static tryParseHeader(header: string): MutableGCodeInfo | null {
		...
		if (match) {
			...
			let raticalDialectVersion: string | undefined = undefined;
			...
			switch (match.groups?.GENERATOR?.toLowerCase()) {
				...
				case 'superslicer':
					...
					break;
				default:
					if (match.groups?.RATICAL_DIALECT_VERSION) {
						flavour = GCodeFlavour.Ratical;
						raticalDialectVersion = match.groups?.RATICAL_DIALECT_VERSION;
					}
					break;
			}
			...
			let pbrRaticalMetaFileOffset: number | undefined;
			let pbrIdex: boolean | undefined;

			rxProcessedByRaticalHeaderV3.lastIndex = 0;
			const pbrMatch = rxProcessedByRaticalHeaderV3.exec(header);

			if (pbrMatch) {
				pbrVersion = coerceSemVerOrThrow(
					pbrMatch?.groups?.VERSION,
					'The processed by Ratical version is not a valid SemVer.',
				);
				...
				pbrRaticalMetaFileOffset = pbrMatch.groups?.M ? parseInt(pbrMatch.groups?.M, 16) : undefined;
				...
				rxProcessedByRaticalHeaderPreV3.lastIndex = 0;
				const pbrMatch = rxProcessedByRaticalHeaderPreV3.exec(header);
			...
			return new MutableGCodeInfo(
				...
				coerceSemVerOrThrow(raticalDialectVersion, 'The Ratical dialect version is not a valid SemVer.'),
				pbrVersion,
				pbrTimestamp,
				undefined,
				pbrFileFormatVersion,
				pbrRaticalMetaFileOffset,
				pbrIdex,
			);
		}
	...
	/** Gets the 'Processed by Ratical' header for the current code version. */
	public static getProcessedByRaticalHeader(args: {
		currentCodeVersion: semver.SemVer;
		timestamp: Date;
		raticalMetaFileOffset: number;
		processedForIdex: boolean;
	}): string {
		return `; processed by Ratical.PostProcessor ${args.currentCodeVersion.toString()} on ${date2.format(args.timestamp, 'YYYY-MM-DD [at] HH:mm:ss [UTC]', true)} v:${GCodeFile.FILE_FORMAT_VERSION} m:${args.raticalMetaFileOffset.toString(16)}${args.processedForIdex ? ' idex' : ''}`;
	}
}
