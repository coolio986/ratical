 ...
 * @description The Ratical gcode post-processor.
 *
...
import { Action, ActionFilter, REMOVED_BY_RATICAL } from '@/server/gcode-processor/Actions';
...
export interface FinalizeProcessingOptions {
	...
	getProcessedByRaticalHeader: (currentCodeVersion: SemVer, timestamp: Date) => string;
}
		...
		while (i < flat.length) {
			...
			if ((flavour & gcodeInfo.flavour) > 0) {
				...
				if (typeof semVerRange === 'string') {
					...
					if (
						semver.satisfies(
							flavour == GCodeFlavour.Ratical ? gcodeInfo.raticalDialectVersion! : gcodeInfo.generatorVersion!,
							semVerRange,
						)
					) {
						return true;
	...
	async finalizeProcessing(options?: FinalizeProcessingOptions): Promise<MutableGCodeInfo> {
		...
		if (s.firstLine) {
			await options.replaceLine(
				...
				options.getProcessedByRaticalHeader(currentPPVersion, now) + '\n' + s.firstLine.line.trimEnd(),
			);
		}
				...
				if (s.extruderTempLines) {
					for (let bmLine of s.extruderTempLines) {
						await options.replaceLine(
							...
							REMOVED_BY_RATICAL + bmLine.line.trimEnd(),
						);
					}
				}
			}
