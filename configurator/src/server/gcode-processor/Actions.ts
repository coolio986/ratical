...
export const CHANGED_BY_RATICAL = ' ; Changed by Ratical post processor: ';
export const REMOVED_BY_RATICAL = '; Removed by Ratical post processor: ';
 ...
 *   the action applies. If `flavour` is {@link GCodeFlavour.Ratical}, then then {@link GCodeInfo.raticalDialectVersion} is compared, otherwise
		...
		switch (gcodeInfo.flavour) {
			case GCodeFlavour.Unknown:
				throw new SlicerNotSupported(
					`Slicer '${gcodeInfo.generator}' is not supported, and Ratical dialect conformance was not declared.`,
					{ cause: gcodeInfo },
				);
			...
			case GCodeFlavour.SuperSlicer:
				...
				break;
			case GCodeFlavour.Ratical:
				if (semver.neq('0.1', gcodeInfo.generatorVersion)) {
					throw new SlicerNotSupported(
						`Only version 0.1 of the Ratical G-code dialect is supported. Version ${gcodeInfo.generatorVersion} is not supported.`,
						{ cause: gcodeInfo },
					);
				}
				break;
			...
			if (/^_ON_LAYER_CHANGE LAYER=2($|[\s;])/i.test(c.line)) {
				...
				for (let scan of c.scanForward(9)) {
					if (scan.line.startsWith('M104 S')) {
						s.extruderTempLines ??= [];
						scan.line = scan.line.padEnd(scan.line.length + REMOVED_BY_RATICAL.length);
			...
			for (let scan of c.scanBack(19)) {
				...
				if (cmd && cmd.letter === 'G' && cmd.value === '1') {
					...
					if (
						...
						scan.prepend(REMOVED_BY_RATICAL);
					}
				}
			}
				...
				if (cmd && cmd.letter === 'G' && cmd.value === '1') {
					...
					if (!s.hasPurgeTower) {
						if (cmd.e) {
							...
							prevZMove?.prepend(REMOVED_BY_RATICAL);
							prevZMove = scan;
							if (!xyMoveAfterToolchange) {
								++zMoveCount1;
							} else {
