...
export class ToolheadGenerator<IsToolboard extends boolean> extends ToolheadHelper<IsToolboard> {
	...
	public renderHotend(controlboard: Board) {
		...
		result.push(
			`# ${this.getToolCommand()} ${this.getHotend().title} definition (from Ratical/hotends/${this.getHotend().id}.cfg)`,
		);
	...
	public renderExtruder() {
		...
		result.push(
			`# ${this.getToolCommand()} ${this.getExtruder().title} definition (from Ratical/extruders/${
				this.getExtruder().id
			}.cfg)`,
		);
