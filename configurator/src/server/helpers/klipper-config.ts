...
import {
	...
	VAOCControlPoints,
} from '@/server/helpers/config-generation/ratical-vaoc';
...
export const constructKlipperConfigExtrasGenerator = (
	...
	return {
		...
		generateSaveVariables(options?: VAOCControlPoints) {
			...
			return [
				{
					fileName: 'ratical-variables.cfg',
					content: vars.join('\n'),
					overwrite: false,
				},
			].map((f) => {
...
export const constructKlipperConfigHelpers = async (
	...
	return {
		...
		renderBoards() {
			...
			const result: string[] = [
				'[include Ratical/boards/rpi/config.cfg]', // Always include RPi.
		...
		renderBoardQuirks() {
			...
			if (config.controlboard.hasQuirksFiles) {
				...
				if (utils.getToolhead(PrinterAxis.extruder)?.getToolboard() != null) {
					result.push(`[include Ratical/boards/${config.controlboard.id}/quirks-toolboard.cfg]`);
				} else {
					result.push(`[include Ratical/boards/${config.controlboard.id}/quirks.cfg]`);
				}
			}
			utils.getToolheads().forEach((th) => {
				...
				if (toolboard?.hasQuirksFiles) {
					...
					result.push(`[include Ratical/boards/${toolboard.id}/quirks.cfg]`);
				}
			});
			return result.join('\n');
		},
		...
		renderInputShaper(printerSize: z.output<typeof PrinterSizeDefinition>) {
			...
			if ([xToolhead.getYAccelerometerName(), xToolhead.getXAccelerometerName()].includes('rpi')) {
				...
				result.push(`[include Ratical/sensors/rpi-adxl345.cfg]`);
				result.push(''); // Add a newline for readability.
			}
			...
			// Ratical-equivalent slow sweep (Kalico defaults sweeping_period to 0)
			result.push('sweeping_period: 1.2');
			result.push('sweeping_accel: 400');
		...
		renderProbeIncludes() {
			...
			if (th) {
				result.push(`[include Ratical/z-probe/${th.getProbe()?.id + '.cfg'}]`);
			}
				...
				switch (th.getProbe()?.id) {
					...
					case 'beacon':
						...
						if (!onlyPins) {
							...
							reminder.push('# Run BEACON_RATICAL_CALIBRATE for automatic calibration.');
		...
		renderZOffsetGuidance(additionalLinePrefix: string = '') {
			...
			if (config.toolheads.some((th) => th.probe?.id === 'beacon')) {
				result.push(
					`Z-offset calibration: run BEACON_RATICAL_CALIBRATE to automatically calibrate your beacon for scan and contact.`,
				);
				result.push(
		...
		renderEndstopSection(pretunedSensorlessConfig?: string) {
			...
			toolheads.forEach((th) => {
				if (th.getXEndstop().id !== 'sensorless') {
					...
					result.push(`[gcode_macro Ratical]`);
		...
		async renderToolheadAlignmentSystemHardwareAsync() {
			// This section is typically emitted in Ratical.cfg (but only for IDEX printers)
		...
		renderBase() {
			return [`[include Ratical/homing.cfg]`, `[include Ratical/macros.cfg]`, `[include Ratical/shell-macros.cfg]`].join(
				'\n',
			);
		},
		renderReminders() {
