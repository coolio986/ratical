...
const PRINTER_SETTINGS_FILE = path.join(environment.RATICAL_DATA_DIR, 'last-printer-settings.json');
...
export const savePrinterSettings = async (serializedConfig: SerializedPrinterConfiguration) => {
	return await writeFile(
		path.join(environment.RATICAL_DATA_DIR, 'last-printer-settings.json'),
