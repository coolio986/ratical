...
export const WifiSetup: React.FC<StepScreenProps> = (props) => {
	...
	const [hostname, setHostname] = useState('Ratical');
	...
	const content =
		...
		!rebootMutation.isError ? (
			<Modal
				...
				body={`Ratical is now setup to connect to ${selectedNetwork.ssid}! Your raspberry pi will now reboot, and join your local wifi network. Click the button below to reboot the pi and close this window. You can then reconnect to your local network where http://${hostname}.local/ should be available in a few minutes. If Ratical fails to join ${selectedNetwork.ssid}, it will recreate the "ratical" hotspot and you'll have to try again.`}
				content={
			...
			<TextInput
				...
				help='Only use characters from a-Z and dashes. For example, entering "Ratical" will make your printer available at http://Ratical.local/'
			/>
		) : selectedNetwork ? (
