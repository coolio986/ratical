	...
	if (boardVersion.error && !forceReflash) {
		...
		content = (
			<Fragment>
				...
				<p>Please wait while Ratical queries your board..</p>
			</Fragment>
		);
		...
		const path = (
			<Card className="flex flex-col justify-between">
				<CardHeader>
					...
					<CardDescription>
						If Ratical has already detected your board, it can be flashed automatically. This is the fastest and easiest
						method.
					</CardDescription>
				</CardHeader>
				<CardContent className="text-right xl:text-left">
		...
		const unidentifiedPath = (
			<Card className="flex flex-col justify-between">
				<CardHeader>
					...
					<CardDescription>
						If Ratical detects any boards that are already running klipper but not managed by Ratical, their device path
				...
				case 'path':
					content = (
						<div>
							...
							<div className="prose mt-4 text-base text-zinc-500">Please wait while Ratical flashes your board.</div>
