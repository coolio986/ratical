...
export const moonrakerWriteEffect = <T extends any = unknown>(): AtomEffect<T> => {
	return (params) => {
		params.onSet((newValue) => {
			...
			// 		value: newValue,
			// 	},
			// 	`Ratical Atom Sync Effect: new value was saved to moonraker "${params.trigger}"`,
			// );
...
export const SyncWithMoonraker: React.FC<React.PropsWithChildren> = ({ children }) => {
	...
	const read: ReadItem = useCallback(
		async (itemKey) => {
			const value = await moonraker.getItem('Ratical', itemKey as '__recoil');
	...
	const saveAtom = useCallback(
		async (event: Event) => {
			...
			await moonraker.saveItem('Ratical', itemKey as '__recoil', value);
		},
		[moonraker],
	);
