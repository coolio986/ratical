...
describe('configuration', async () => {
	...
	const scripts = (await promisify(fs.readdir)(path.join(environment.RATICAL_CONFIGURATION_PATH, 'scripts'))).filter(
	...
	test.concurrent('is free of dumb typos', async () => {
		const files = await glob(environment.RATICAL_CONFIGURATION_PATH + '/**/*.cfg');
	...
	describe('has executable bash scripts', async () => {
		test.concurrent.each(scripts)('%s is executable', async (script) => {
			await promisify(fs.access)(path.join(environment.RATICAL_CONFIGURATION_PATH, 'scripts', script), fs.constants.X_OK);
		});
	});
