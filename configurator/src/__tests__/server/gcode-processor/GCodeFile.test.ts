...
const defaultExpectedInfo = {
	raticalDialectVersion: undefined,
	...
	raticalMetaFileOffset: undefined,
};
...
describe('tryParseHeader', async () => {
	test.each([
		...
		[
			'CustomGenerator in Ratical dialect',
			'; generated by CustomGenerator 0.1-alpha in Ratical dialect 0.1 on 2025-01-01 at 08:32:07 UTC\n\n',
			{
				...defaultExpectedInfo,
				...
				raticalDialectVersion: semver.coerce('0.1'),
			},
		],
		[
			'CustomGenerator without Ratical dialect',
		...
		[
			...
			'; generated by PrusaSlicer 2.8.0+win64 on 2024-09-06 at 08:32:07 UTC\n' +
				'; processed by Ratical 2.0.2-518-g4ffef464 on 2025-01-01 at 08:30:00Z\n',
			{
				...defaultExpectedInfo,
				fileFormatVersion: 1,
				flavour: GCodeFlavour.PrusaSlicer,
	...
	test('getProcessedByRaticalHeader', async () => {
		let header = GCodeFile.getProcessedByRaticalHeader({
			currentCodeVersion: new SemVer('1.2.3'),
			timestamp: new Date(0xbadbabe),
			raticalMetaFileOffset: 0xdecafbad,
			processedForIdex: false,
		});
		expect(header).toEqual('; processed by Ratical.PostProcessor 1.2.3 on 1970-01-03 at 06:25:34 UTC v:3 m:decafbad');

		header = GCodeFile.getProcessedByRaticalHeader({
			currentCodeVersion: new SemVer('1.2.3'),
			timestamp: new Date(0xbadbabe),
			raticalMetaFileOffset: 0xdecafbad,
		...
		expect(header).toEqual(
			'; processed by Ratical.PostProcessor 1.2.3 on 1970-01-03 at 06:25:34 UTC v:3 m:decafbad idex',
		);
	});
});
	...
	test.each([
		...
		[
			...
			path.join(__dirname, 'fixtures', 'other', 'legacy_processed.gcode'),
			{
				...
				printabilityReasons: [
					expect.stringContaining('The file format is from an old version of Ratical which is no longer supported'),
				],
				info: {
					...defaultExpectedInfo,
					fileFormatVersion: 0,
		...
		[
			'without ratical_meta block, format v3',
			path.join(__dirname, 'fixtures', 'other', 'without_ratical_meta.gcode'),
			{},
			{ expectedWarnings: ['INVALID_METADATA: The ratical_meta block was not found.'] },
		],
		[
			'without ratical_meta begin, format v3',
			path.join(__dirname, 'fixtures', 'other', 'without_ratical_meta_begin.gcode'),
			{},
			{ expectedWarnings: ['INVALID_METADATA: Failed to parse ratical_meta block: the begin marker was not found.'] },
		],
		[
			'wrong ratical_meta data length, format v3',
			path.join(__dirname, 'fixtures', 'other', 'ratical_meta_wrong_data_length.gcode'),
			{},
			{
				expectedWarnings: [
					'INVALID_METADATA: Failed to parse ratical_meta block: expected 999 base64 characters, but found 34.',
				],
			},
		],
