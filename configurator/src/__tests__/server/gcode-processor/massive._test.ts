 ...
 * stats etc:
 *
 * `~/Ratical-dev/Ratical-configurator/src$ /usr/bin/time -v pnpm vitest run --no-ui massive`
 *
 */
describe('massive', async () => {
	test('massive', { timeout: 999999999 }, async () => {
		const gci = await GCodeFile.inspect('/mnt/c/dev/ratical-gcode-samples/massive.gcode', {});
		await gci.transform('/mnt/c/dev/ratical-gcode-samples/massive.out.gcode', { printerHasIdex: true });
	});
});
