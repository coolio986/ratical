...
async function processedGCodeFilesAreEquivalent(expectedPath: string, actualPath: string) {
	...
	try {
		...
		expect(gciActual!.raticalDialectVersion).to.equal(gciExpected!.raticalDialectVersion, ' while comparing headers');
		...
		while (true) {
			...
			if (actual.value.startsWith('; ratical_meta begin ') && expected.value.startsWith('; ratical_meta begin ')) {
				// Stop comparing. We can't meaningfully compare ratical_meta blocks. This is handled by other tests.
				break;
			}
