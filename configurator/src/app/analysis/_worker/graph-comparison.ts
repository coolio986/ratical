...
export const detectPeaks = (psd: PSD, detectionThreshold: number): Peak[] => {
	const avgValues = psd.estimates.map((value, index, values) => {
	...
	for (let i = 1; i < avgValues.length - 1; i++) {
		const prev = avgValues[i - 1];
		const current = avgValues[i];
		const next = avgValues[i + 1];
