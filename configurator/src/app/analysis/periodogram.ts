...
/** Plain min/max range for PSD math. */
const powerRange = (min: number, max: number) => ({ min, max });
...
export async function powerSpectralDensity(
	...
	const data = x.map((series) => {
		...
		return {
			...
			powerRange: powerRange(minPower, maxPower),
		};
	});
...
export const sumPSDs = (PSDs: TypedArrayPSD[]): TypedArrayPSD => {
	...
	return {
		estimates: sum,
		frequencies: Float64Array.from(PSDs[0].frequencies),
		powerRange: powerRange(Math.min(...sum), Math.max(...sum)),
	};
};
