...
export const program = new commander.Command()
	...
	.description('Ratical CLI for interacting with the Ratical Configurator')
...
program
	.command('info')
	.description('Print info about this Ratical installation')
	.action(async () => {
...
const extensions = program
	.command('extensions')
	.description('Register, unregister or symlink extensions managed by the Ratical Configurator');

const registerExtensions = extensions
	.command('register')
	.description('Register an extension to be managed by the Ratical Configurator');

const unregisterExtensions = extensions
	.command('unregister')
	.description('Unregister an extension from the Ratical Configurator');
...
registerExtensions
	.command('klipper')
	.description('Register a Klipper extension to be managed by the Ratical Configurator')
...
registerExtensions
	.command('moonraker')
	.description('Register a Moonraker extension to be managed by the Ratical Configurator')
	.argument('<name>', 'Name of the extension')
...
unregisterExtensions
	.command('klipper')
	.description('Unlink and unregister a Klipper extension managed by the Ratical Configurator')
	.argument('<name>', 'Name of the extension')
	.showHelpAfterError()
...
unregisterExtensions
	.command('moonraker')
	.description('Unlink and unregister a Moonraker extension managed by the Ratical Configurator')
	.argument('<name>', 'Name of the extension')
	.showHelpAfterError()
	.option('-e, --error-if-not-exists', "Throw error if the extension doesn't exist")
	.action(async (extName, options) => {
...
program
	.command('config')
	.description('Commands for managing the Ratical configuration')
	.command('regenerate')
...
const log = program.command('logs').description('Commands for managing the Ratical log');

log
	.command('tail')
	.option('-f, --follow', 'Follow the log')
	.option('-n, --lines <lines>', 'Number of lines to show')
	.description('Tail the Ratical log')
	.action(async (options) => {
		const $$ = $({ verbose: true });
		const flags = [];
		if (options.follow) {
...
log
	.command('rotate')
	.description('force rotate the Ratical configurator log')
	.action(async () => {
		const log = '/etc/logrotate.d/ratical-configurator';
...
const doctor = program
	.command('doctor')
	.description('Diagnose and fix common issues on a Ratical installation')
	.action(async () => {
		await ensureSudo();
		...
		let { rerender } = render(
			<InstallProgressUI
				status="Fixing potential Ratical issues..."
				stepText="Repairing Ratical configurator..."
				isLoading={true}
				cmdSignal={cmdSignal}
				steps={steps}
			/>,
		...
		await $$`sudo ${loadEnvironment().RATICAL_SCRIPT_DIR}/update.sh`;
		steps.push({ name: 'Repaired Ratical configurator', status: 'success' });
		rerender(
			<InstallProgressUI
				status="Fixing potential Ratical issues..."
				stepText="Restarting Ratical configurator..."
				cmdSignal={cmdSignal}
				isLoading={true}
				steps={steps}
			/>,
		);
		await $$`sudo systemctl restart ratical-configurator`;
		...
		steps.push({ name: 'Restarted Ratical configurator', status: 'success' });
		rerender(
			<InstallProgressUI
				status="Fixing potential Ratical issues..."
				stepText="Repairing Ratical configuration..."
				isLoading={true}
				cmdSignal={cmdSignal}
				steps={steps}
			/>,
		);
		await $$`sudo ${loadEnvironment().RATICAL_CONFIGURATION_PATH}/scripts/ratical-update.sh`;
		steps.push({ name: 'Repaired Ratical configuration', status: 'success' });
		rerender(
			<InstallProgressUI
				status="Fixing potential Ratical issues..."
