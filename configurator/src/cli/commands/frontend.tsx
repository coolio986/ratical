...
export const frontend = (program: Command) => {
	...
	frontend
		...
		.description('Use experimental Ratical fork of Fluidd')
		.action(async (channel) => {
			...
			if (!existsSync(mainsailConfigFile)) {
				...
				if (!existsSync(`/home/${environment.USER}/fluidd`)) {
					// Download and unpack latest Ratical fluidd release
					rerender(
						<InstallProgressUI
							cmdSignal={cmdSignal}
							steps={steps}
							...
							isLoading={true}
							stepText="Downloading latest Ratical Fluidd release"
						/>,
					);
					await $$`wget https://github.com/fluidd-core/fluidd/releases/latest/download/fluidd.zip -O /tmp/fluidd.zip`;
					steps.push({ name: 'Download Ratical Fluidd', status: 'success' });
					rerender(
						<InstallProgressUI
							cmdSignal={cmdSignal}
							steps={steps}
					...
					steps.push({ name: 'Extract Ratical Fluidd', status: 'success' });
				} else {
					steps.push({
				...
				if (!existsSync(`/home/${environment.USER}/printer_data/config/.fluidd-theme`)) {
					...
					rerender(
						<InstallProgressUI
							...
							status="Installing Fluidd..."
							isLoading={true}
							stepText="Cloning Ratical fluidd theme"
						/>,
					);
					await $$`git clone https://github.com/coolio986/fluidd-theme /home/${environment.USER}/printer_data/config/.fluidd-theme`;
					steps.push({ name: 'Clone Ratical fluidd theme', status: 'success' });
				} else {
					steps.push({
						name: 'Ratical fluidd theme is already installed, git cloning has been skipped.',
						status: 'warning',
					});
				}
				...
				rerender(
					<InstallProgressUI
						...
						status="Installing Fluidd..."
						isLoading={true}
						stepText="Adding moonraker entry for Ratical Fluidd fork"
					/>,
				);
				const fluiddUpdateSection = `\n[update_manager Fluidd]\ntype: web\nrepo: fluidd-core/fluidd\npath: ~/fluidd\n${channel === 'beta' ? 'channel: beta\n' : 'channel: stable\n'}info_tags:\n\tdesc: Experimental: Fluidd Web Interface (Ratical Fork)`;
				...
				rerender(
					<InstallProgressUI
						...
						status="Installing Fluidd..."
						isLoading={true}
						stepText="Adding moonraker entry for Ratical Fluidd theme"
					/>,
				);
				const fluiddThemeUpdateSection = `\n[update_manager FluiddTheme]\ntype: git_repo\npath: ~/printer_data/config/.fluidd-theme\nprimary_branch: main\norigin: https://github.com/coolio986/fluidd-theme\nis_system_service: false\ninfo_tags:\n\tdesc: Experimental: Ratical Fluidd Theme`;
