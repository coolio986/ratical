/** download-firmware.ts — Legacy Next.js pages/ API route. See docs/ARCHITECTURE.md §4. */
	...
	if (req.method === 'GET') {
		...
		if (process.env.RATICAL_CONFIGURATION_PATH == null) {
			return res.status(500).json({
				result: 'error',
				data: {
					message: 'RATICAL_CONFIGURATION_PATH environment variable not set',
				},
			});
		}
		const environment = serverSchema.parse(process.env);
		const firmwarePath = path.join(environment.RATICAL_DATA_DIR, board.firmwareBinaryName);
		try {
