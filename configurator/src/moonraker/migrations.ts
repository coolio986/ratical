/** migrations.ts — Moonraker API client (websocket/DB access). See docs/ARCHITECTURE.md §4. */
...
export const migrations: Migration[] = [
	{
		version: 1,
		description: "Undo double JSON encoding of Ratical's data",
		up: async () => {
			const res = await window.fetch(`http://${getHost()}/server/database/item?namespace=Ratical`);
			if (res.ok) {
				...
				for await (const [key, value] of Object.entries(data.result.value)) {
					try {
						...
						await window.fetch(
							`http://${getHost()}/server/database/item?namespace=Ratical&key=${encodeURIComponent(key)}`,
							{
								method: 'DELETE',
								headers: {
									'Content-Type': 'application/json',
...
export const getCurrentVersion = async (): Promise<number> => {
	const res = await window.fetch(`http://${getHost()}/server/database/item?namespace=Ratical&key=__db_version`);
	if (res.ok) {
